import ut,SALT28;
export hygiene(dataset(layout_TravelDataCombined2) h) := MODULE
...
export Summary(SALT28.Str30Type txt) := FUNCTION
  SummaryLayout := RECORD
    ...
    populated_Booker_First_Name_pcnt := AVE(GROUP,IF(h.Booker_First_Name = (TYPEOF(h.Booker_First_Name))'',0,100));
    maxlength_Booker_First_Name := MAX(GROUP,LENGTH(TRIM((SALT28.StrType)h.Booker_First_Name)));
    avelength_Booker_First_Name := AVE(GROUP,LENGTH(TRIM((SALT28.StrType)h.Booker_First_Name)),h.Booker_First_Name<>(typeof(h.Booker_First_Name))'');
    populated_Booker_Last_Name_pcnt := AVE(GROUP,IF(h.Booker_Last_Name = (TYPEOF(h.Booker_Last_Name))'',0,100));
    maxlength_Booker_Last_Name := MAX(GROUP,LENGTH(TRIM((SALT28.StrType)h.Booker_Last_Name)));
    avelength_Booker_Last_Name := AVE(GROUP,LENGTH(TRIM((SALT28.StrType)h.Booker_Last_Name)),h.Booker_Last_Name<>(typeof(h.Booker_Last_Name))'');
    populated_Email_Address_pcnt := AVE(GROUP,IF(h.Email_Address = (TYPEOF(h.Email_Address))'',0,100));
    maxlength_Email_Address := MAX(GROUP,LENGTH(TRIM((SALT28.StrType)h.Email_Address)));
    avelength_Email_Address := AVE(GROUP,LENGTH(TRIM((SALT28.StrType)h.Email_Address)),h.Email_Address<>(typeof(h.Email_Address))'');
  END;
...
SHARED FldIds := DATASET([{1,'Event'}
      ...
      ,{26,'Special_requests'}
      ,{27,'Share_With'}
      ,{28,'Booker_First_Name'}
      ,{29,'Booker_Last_Name'}
      ,{30,'Email_Address'}],SALT28.MAC_Character_Counts.Field_Identification);
...
ErrorRecord NoteErrors(h le,unsigned1 c) := transform
  self.ErrorNum := CHOOSE(c,
    ...
    Fields.InValid_Booker_First_Name((SALT28.StrType)le.Booker_First_Name),
    Fields.InValid_Booker_Last_Name((SALT28.StrType)le.Booker_Last_Name),
    Fields.InValid_Email_Address((SALT28.StrType)le.Email_Address),
    0);
