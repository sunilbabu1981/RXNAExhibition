...
EXPORT SALT28.StrType FieldName(UNSIGNED2 i) := CHOOSE(i,'Show','Year','Category','Attendance_Id','Group_Id','Last_name','First_name','Company','Address1','Address2','City','State','Postal_Code','Country','Email','Phone','Method_of_Housing','Hotel','Arrive','Depart','Hotel_Confirmation','Payment_type','Room_type','Cost','Comments','Special_requests','Share_With');
EXPORT FieldNum(SALT28.StrType fn) := CASE(fn,'Show' => 1,'Year' => 2,'Category' => 3,'Attendance_Id' => 4,'Group_Id' => 5,'Last_name' => 6,'First_name' => 7,'Company' => 8,'Address1' => 9,'Address2' => 10,'City' => 11,'State' => 12,'Postal_Code' => 13,'Country' => 14,'Email' => 15,'Phone' => 16,'Method_of_Housing' => 17,'Hotel' => 18,'Arrive' => 19,'Depart' => 20,'Hotel_Confirmation' => 21,'Payment_type' => 22,'Room_type' => 23,'Cost' => 24,'Comments' => 25,'Special_requests' => 26,'Share_With' => 27,0);
//Individual field level validation
EXPORT Make_Show(SALT28.StrType s0) := s0;
EXPORT InValid_Show(SALT28.StrType s) := FALSE;
EXPORT InValidMessage_Show(UNSIGNED1 wh) := '';
EXPORT Make_Year(SALT28.StrType s0) := s0;
EXPORT InValid_Year(SALT28.StrType s) := FALSE;
EXPORT InValidMessage_Year(UNSIGNED1 wh) := '';
...
#uniquename(fd)
  %dl% %fd%(in_left le,in_right ri) := TRANSFORM
    SELF.Diff_Show := le.Show <> ri.Show;
    ...
    SELF.Num_Diffs := 0+ IF( SELF.Diff_Show,1,0)+ IF( SELF.Diff_Year,1,0)+ IF( SELF.Diff_Category,1,0)+ IF( SELF.Diff_Attendance_Id,1,0)+ IF( SELF.Diff_Group_Id,1,0)+ IF( SELF.Diff_Last_name,1,0)+ IF( SELF.Diff_First_name,1,0)+ IF( SELF.Diff_Company,1,0)+ IF( SELF.Diff_Address1,1,0)+ IF( SELF.Diff_Address2,1,0)+ IF( SELF.Diff_City,1,0)+ IF( SELF.Diff_State,1,0)+ IF( SELF.Diff_Postal_Code,1,0)+ IF( SELF.Diff_Country,1,0)+ IF( SELF.Diff_Email,1,0)+ IF( SELF.Diff_Phone,1,0)+ IF( SELF.Diff_Method_of_Housing,1,0)+ IF( SELF.Diff_Hotel,1,0)+ IF( SELF.Diff_Arrive,1,0)+ IF( SELF.Diff_Depart,1,0)+ IF( SELF.Diff_Hotel_Confirmation,1,0)+ IF( SELF.Diff_Payment_type,1,0)+ IF( SELF.Diff_Room_type,1,0)+ IF( SELF.Diff_Cost,1,0)+ IF( SELF.Diff_Comments,1,0)+ IF( SELF.Diff_Special_requests,1,0)+ IF( SELF.Diff_Share_With,1,0);
  END;
...
#uniquename(AggRec)
  %AggRec% := RECORD
    Count_Diff_Show := COUNT(GROUP,%Closest%.Diff_Show);
